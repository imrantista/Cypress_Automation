      ...
      cy.get("body").then(($body) => {
        if ($body.find(editButtonSelector).length > 0) {
          cy.get(editButtonSelector)
            ...
            .then(($btn) => {
              if ($btn.length > 0) {
                const errorMessage = "Error: Host able to see product edit button";
                resultTracker.errorMessages.push(errorMessage);
                resultTracker.errorCount++;
                const stepToReproduce =
                  "Login with host user and try to edit product";
                resultTracker.stepToReproduces.push(stepToReproduce);
                cy.log(errorMessage);
              }
            });
        } else {
          cy.log('Success: "Host not able to see product edit button"');
          resultTracker.successCount++;
        }
      });
    }
  }
  export default ProductEditPermissions;
