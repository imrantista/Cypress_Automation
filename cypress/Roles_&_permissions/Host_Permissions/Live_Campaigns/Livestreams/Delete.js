class LivestreamDeletePermissions {
      ...
      cy.get("body").then(($body) => {
        if ($body.find(deleteButtonSelector).length > 0) {
          cy.get(deleteButtonSelector)
            ...
            .then(($btn) => {
              if ($btn.length > 0) {
                const errorMessage = "Error: Host able to see Livestream delete button";
                resultTracker.errorMessages.push(errorMessage);
                resultTracker.errorCount++;
                const stepToReproduce =
                  "Login with host user and try to delete Livestream";
                resultTracker.stepToReproduces.push(stepToReproduce);
                cy.log(errorMessage);
              }
            });
        } else {
          cy.log('Success: "Host not able to see Livestream delete button"');
          resultTracker.successCount++;
        }
      });
    }
  }
  export default LivestreamDeletePermissions;
